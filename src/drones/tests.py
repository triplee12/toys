...
class PilotTests(APITestCase):
    ...
    def create_user_and_set_token_credentials(self):
        user = User.objects.create_user(
            'tester01', 'tester01@gmail.com', 'tester01'
        )
    ...
    def test_post_and_get_pilot(self):
        ...
        Ensure we cannot retrieve the persisted pilot witout a token
        """
        ...
        print(response)
        print(f"nPK {0}n".format(Pilot.objects.get().pk))
...
class DroneCategoryTests(APITestCase):
    ...
    def test_post_and_get_drone_category(self):
        """
        Ensure we can create a new DroneCategory and the retrieve it
        """
        new_drone_category_name = "Hexacopter"
        response = self.post_drone_category(new_drone_category_name)
        print(f"PK {0}".format(DroneCategory.objects.get().pk))
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_get_drone_categories_collections(self):
        """
